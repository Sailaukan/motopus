...
const EditorPage: React.FC = () => {
    ...
    const generateImages = async (keyword: string): Promise<string[]> => {
        const URL = `https://pixabay.com/api/?key=${process.env.NEXT_PUBLIC_PIXABAYAPIKEY}&q=${encodeURIComponent(keyword)}&per_page=10`;
    ...
    overflow-y-auto`}>
                    {userId ?
                        <div>
                            ...
                            <ul className="space-y-2 mb-4">
                                {videoCommands.slice().reverse().map((command, index) => (
                                    ...
                                    >
                                        <div
                                            className={`inline-block ${command.prompt.length > 23 ? 'hover:animate-marquee' : ''
                                                }`}
                                        >
                                            {command.prompt}
                                        </div>
                        ...
                        <div className="mt-10 lg:mt-14">
                            ...
                            <button onClick={() => router.push('/sign-up')} className="bg-purple-500 w-full text-white px-3 py-3 lg:px-5 lg:py-3 rounded-md text-xs sm:text-sm hover:bg-purple-700 transition-colors shadow-lg">
                                Sign Up
                            </button>
                        </div>
                    }
